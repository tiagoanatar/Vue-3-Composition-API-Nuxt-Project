...
const cxRadio = computed(() => [
  'form-check',
  'theme-radio-card-button d-flex align-items-center',
]);
...
<template>
  ...
  <div :class="cxRadio" @click="clickHandler">
    <BaseSkeletonLoader :is-loading="isLoading" :rows="[{ col: 3 }]">
      ...
      >
        <span class="theme-radio-card-button-title">
          {{ label }}
        </span>
        <span class="theme-radio-card-button-description">
          {{ description }}
        </span>
      </label>
      <div class="theme-radio-card-button-icon">
        <BaseIcon :name="IconNames.CreditCard" />
      </div>
    </BaseSkeletonLoader>
  </div>
