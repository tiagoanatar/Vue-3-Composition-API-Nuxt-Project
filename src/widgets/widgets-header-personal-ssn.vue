...
const content = ref({
  backCta: Routes.PersonalAddress,
  backLabel: 'back',
  dataTestId: 'personal-ssn-header',
});

onMounted(async () => {
  const data = await getPersonalInfo();
  const stateCode = data?.homeAddress?.stateCode || '';

  if (['FL', 'MI'].includes(stateCode)) {
    content.value.backCta = Routes.PersonalIdentity;
  }
});
</script>
